...
import module namespace visjs = "http://marklogic.com/visjs/graph" at "/ext/visjs-graph/visjs-lib.xqy";
...
{
  xdmp:trace("lvbb-vis", $params),
  let $subjects := map:get($params, "subjects")
  let $result := visjs:build-graph($subjects)
  return (
    xdmp:trace("lvbb-vis", $result),
    $result
  )
};
